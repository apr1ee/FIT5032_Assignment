    ...
    <div class="col-12 col-md-6 col-lg-5">
      ...
      <form @submit.prevent="onSubmit" novalidate>
        <div class="mb-3">
          ...
          <input type="email" class="form-control" :class="{'is-invalid':t.email&&err.email}"
                 v-model.trim="f.email" placeholder="you@example.com" @blur="t.email=true" />
          <div class="invalid-feedback" v-if="t.email&&err.email">{{ err.email }}</div>
        </div>

        <div class="mb-3">
          <label class="form-label">Password</label>
          <input type="password" class="form-control" :class="{'is-invalid':t.pwd&&err.password}"
                 v-model.trim="f.password" placeholder="••••••••" @blur="t.pwd=true" />
        ...
        <div class="mb-3" v-if="mode==='register'">
          ...
          <div class="form-text">For demo purposes only—no real server auth.</div>
        </div>
...
const mode = reactive('login')  // 'login' | 'register'
...
const cta = computed(()=> mode==='login'?'Login':'Create account')
const toggleText = computed(()=> mode==='login'?'Need an account? Register':'Have an account? Login')

let flash = ''
function toggle(){ mode==='login' ? mode='register' : mode='login'; flash='' }

async function onSubmit(){
  t.email=true; t.pwd=true; if(hasErrors.value) return
  try{
    if(mode==='login') await login({ email:f.email, password:f.password })
    else await register({ email:f.email, password:f.password, role:f.role })
    router.push(route.query.r || '/')
  }catch(e){ flash = e.message || 'Operation failed.' }
}
</script>
